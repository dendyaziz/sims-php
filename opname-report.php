...
<head>
    ...
    <title>Opname Report :: <?php echo $app_name;?></title>
		...
		<div class="container-fluid">
                ...
                <div class="row page-titles">
                    <div class="col-md-5 align-self-center">
                        <h3 class="text-themecolor">Laporan Stok Opname</h3>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                            <li class="breadcrumb-item active">Laporan Stok Opname</li>
                        </ol>
                    </div>	
													...
													<?php
													    ...
													    if(!empty($pAwal) && !empty($pAkhir)){
															if($branch=="All Branch"){
																$result = $con->query("Select jenis from OPNAME Where tanggal between '$pAwal' and '$pAkhir' group by jenis order By jenis");
															}else{
																$result = $con->query("Select jenis from OPNAME where tanggal between '$pAwal' and '$pAkhir' and branch='$branch' group by jenis order By jenis");
															}
														}else{														
															if($branch=="All Branch"){
																$result = $con->query("Select jenis from OPNAME group by jenis order By jenis");
															}else{
																$result = $con->query("Select jenis from OPNAME where branch='$branch' group by jenis order By jenis");
															}															
														}
														$count = mysqli_num_rows($result);
														if($count>0){						
												...
												<div class="form-group">
													...
													<select class="custom-select col-12" id="kode" name="kode" onchange="this.form.submit()" required="required">
														...
														<?php
															...
															if(!empty($pAwal) && !empty($pAkhir)){
																if($branch=="All Branch"){
																	$result = $con->query("Select kode, barang from OPNAME Where tanggal between '$pAwal' and '$pAkhir' and jenis='$jenis' group by kode, barang order By barang");
																}else{
																	$result = $con->query("Select kode, barang from OPNAME where tanggal between '$pAwal' and '$pAkhir' and jenis='$jenis' and branch='$branch' group by kode, barang order By barang");
																}
															}else{
															
																if($branch=="All Branch"){
																	$result = $con->query("Select kode, barang from OPNAME where jenis='$jenis' group by kode order By barang");
																}else{
																	$result = $con->query("Select kode, barang from OPNAME where jenis='$jenis' and branch='$branch' group by kode order By barang");
																}
					...
					<div class="col-12">
                        <div class="card card-body">
                            <div class="text-right">
								<?php
									echo"<a class='btn btn-success btn-sm text-right' href='export_opname.php?faktur=$faktur&branch=$branch&awal=$awal&akhir=$akhir&jenis=$jenis&kode=$kode'><i class='fa fa-file-excel-o'></i> Export</a>";
									echo" <a class='btn btn-primary btn-sm text-right' href='cetak_opname.php?faktur=$faktur&branch=$branch&awal=$awal&akhir=$akhir&jenis=$jenis&kode=$kode'><i class='fa fa-print'></i> Cetak</a>";						
								?>
							</div>
							<br>
							<div class="col-sm-12 col-xs-12">
								<div class="table-responsive" id="cetak">
									<div class="Sembunyikan">
										<h5 class="card-title"><b>LAPORAN STOK OPNAME</b>				
