...
if(!empty($_GET["id"])){	
	...
	if($count>0){
		while($row = mysqli_fetch_assoc($result))
		{
			...
			$result2 = $con2->query("Insert into HISTORY (branch, faktur, tanggal, transcd, jenis, barang, panjang, lebar, s_out, username, entrydate) values ('$branch', '$faktur', '$tgl_prod', 'I', '$jenis','$barang','$panjang','$lebar','$s_out','$username','$date')");		
